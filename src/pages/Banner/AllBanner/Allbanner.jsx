...
import EditVariantsModal from '../../MenuDiscount/Variants/EditVariantsModal/EditVariantsModal';
...
export default function Allbanner() {
  ...
  const { isSidebarOpen ,loading, setloading, setsearch, search} = useAppContext();
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={`${styles.header}  ${isSidebarOpen ? styles.sidebarOpen : styles.sidebarclose}`}>
          ...
          <button onClick={() => navigate("/CreateBanner")} className={`${styles.addBtn}`}>+ <span className="ms-1">Add New Banner</span></button>
        </div>
            ...
            <tbody>
              {categories.map((cat) => (
                <tr key={cat._id} onClick={() => toggleRow(cat?._id)} >
                  ...
                  <td>
                    <Tooltip text="Edit Banner" position="bottom" bg="black">
        ...
        {loading && <Loader loading={loading} text="Loading Banner..." />}
        <EditVariantsModal
